...
class _ReceiptScannerScreenState extends State<ReceiptScannerScreen> {
  ...
  Future<bool> _ensurePermission(ImageSource source) async {
    ...
    if (source == ImageSource.camera) {
      ...
      // ✅ Gallery permission:
      // - iOS => photos
      // - Android 13+ => READ_MEDIA_IMAGES (mapped by permission_handler)
      // - Older Android => storage fallback
    ...
    if (result.isPermanentlyDenied) {
      await _showOpenSettingsDialog(source);
      return false;
    }
  ...
  Future<void> _showOpenSettingsDialog(ImageSource source) async {
    return showDialog(
      context: context,
  ...
  // ----------------- IMAGE PICKING (Permission First) -----------------
  ...
  double? _extractTotalAmount(String text) {
    ...
    for (final match in numberRegex.allMatches(cleanedText)) {
      ...
      if (val != null && val >= 5.0) {
        // filter out small, likely non-total numbers
        candidates.add(val);
      }
    }
